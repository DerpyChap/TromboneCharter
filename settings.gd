...
@onready var charter 	= %SongInfo.get_node("Charter")
...
@onready var year	 = %SongInfo3.get_node("Year")
@onready var diff	 = %SongInfo3.get_node("Diff")
@onready var notespc = %SongInfo3.get_node("NoteSpacing")

var values : Array:
    get: return [
        title,short_name,author,charter,genre,desc,
	    track_ref,length,tempo,timesig,year,diff,notespc
    ]
